---
name: upload-doc
description: 마크다운 파일을 Linear 프로젝트 Resources에 문서로 추가. "/upload-doc", "리니어에 문서 추가", "프로젝트에 md 파일 올려줘" 등으로 호출.
---
...
```
linear_documentCreate(title, content, projectId?)
```
...
승인 후 `linear_documentCreate` MCP 도구 호출:
- `title`: 추출한 제목
- `content`: 파일 내용
- `projectId`: 선택한 프로젝트 ID
